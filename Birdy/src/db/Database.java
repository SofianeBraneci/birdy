...
public class Database {
	...
	public static Connection getMySQLConnection() throws SQLException {
		...
		if(!DBStatic.pooling) {
			...
			System.out.println(DBStatic.mysql_user);
			// on créer donc une nouvelle connexion
			return DriverManager.getConnection("jdbc:mysql://"+DBStatic.mysql_host+"/"+DBStatic.mysql_db+"?useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=Europe/Paris", DBStatic.mysql_user, DBStatic.mysql_password);
	...
	public static MongoDatabase getMongoDBConnection()  {
		MongoClient client = MongoClients.create(DBStatic.mongo_host);
		MongoDatabase db = client.getDatabase(DBStatic.mongo_bd);
		return db;
	}
